...
export const signin = async (req, res) => {
  ...
  try {
    ...
    if(!existingUser) return res.status(404).json({ message: 'User não existe!' });
...
export const signup = async (req, res) => {
  ...
  try {
    const existingUser = await User.findOne({ email });
    if(existingUser) return res.status(400).json({ message: 'User já existe!' });
    if(password !== confirmPassword) return res.status(400).json({ message: 'As senhas são diferentes' });
